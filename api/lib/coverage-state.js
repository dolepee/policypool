...
const ERROR_CONTRACT = Object.freeze({
  ...
  okx_task_timeline_unavailable: {
    ...
    retryable: false,
    nextAction: "No payment was taken and no task should be recreated. Use the free /api/coverage-preflight event resolver with targetAgent, targetJobId, targetCreatedAt, and jobDescription, or supply the exact creation and acceptance transactions. Use the paid request only if preflight returns eligible.",
  },
  ...
  event_hint_evidence_incomplete: {
    code: "EVENT_HINT_EVIDENCE_INCOMPLETE",
    message: "The X Layer event resolver needs a job id, creation-time hint, and job description.",
    retryable: false,
    nextAction: "Supply every field listed in required. targetAcceptedAt is needed only when acceptance occurred more than 30 minutes after creation.",
  },
  target_creation_time_hint_in_future: {
    code: "CREATION_TIME_HINT_IN_FUTURE",
    message: "The supplied creation-time hint is later than the current X Layer head.",
    retryable: false,
    nextAction: "Correct targetCreatedAt and rerun the free preflight.",
  },
  target_acceptance_time_hint_in_future: {
    code: "ACCEPTANCE_TIME_HINT_IN_FUTURE",
    message: "The supplied acceptance-time hint is later than the current X Layer head.",
    retryable: false,
    nextAction: "Correct targetAcceptedAt and rerun the free preflight.",
  },
  target_event_timestamp_invalid: {
    code: "EVENT_TIME_HINT_INVALID",
    message: "The supplied X Layer event time could not be parsed as a positive timestamp.",
    retryable: false,
    nextAction: "Supply targetCreatedAt and, when needed, targetAcceptedAt as ISO-8601 timestamps.",
  },
  target_event_timeline_invalid: {
    code: "EVENT_TIMELINE_INVALID",
    message: "The acceptance-time hint is earlier than the creation-time hint.",
    retryable: false,
    nextAction: "Correct the event-time hints and rerun the free preflight.",
  },
  target_acceptance_time_hint_required: {
    code: "ACCEPTANCE_TIME_HINT_REQUIRED",
    message: "No unique acceptance event was found within 30 minutes after the verified creation event.",
    retryable: false,
    nextAction: "Add an approximate targetAcceptedAt value. PolicyPool will use it only to locate and then verify the indexed acceptance event.",
  },
  target_event_not_found: {
    code: "TARGET_EVENT_NOT_FOUND",
    message: "No matching X Layer task-escrow event was found near the supplied time hint.",
    retryable: false,
    nextAction: "Check the job id, chain, and time hint. The hint is used only to locate the event; it cannot substitute for one.",
  },
  target_event_ambiguous: {
    code: "TARGET_EVENT_AMBIGUOUS",
    message: "More than one matching task-escrow event was found, so PolicyPool cannot choose evidence safely.",
    retryable: false,
    nextAction: "Supply exact creation and acceptance transaction hashes instead of using event-hint resolution.",
  },
  ...
  target_block_calibration_failed: {
    code: "CHAIN_EVIDENCE_PENDING",
    message: "X Layer block timing could not be calibrated while locating the target job.",
    retryable: true,
    retryAfterSeconds: 10,
    nextAction: "Retry the same request without changing the evidence.",
  },
  target_event_search_window_invalid: {
    code: "CHAIN_EVIDENCE_PENDING",
    message: "The bounded X Layer event search could not be constructed safely.",
    retryable: true,
    retryAfterSeconds: 10,
    nextAction: "Retry the same request without changing the evidence.",
  },
