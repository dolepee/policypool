...
The ownerless `PolicyFeeEscrow` receives the PolicyPool fee before the provider call but cannot send it to the treasury until the provider settlement and clock are quorum-attested. If no provider settlement exists after authorization expiry, the covenant is quorum-cancelled and only the buyer can refund the fee. After a verified settlement, both the buyer path and reconciler use one shared terminalizer: it captures before `refundAvailableAt`, refunds at/after it, rereads concurrent terminal state, and switches a capture that crosses the boundary to refund. If an x402 facilitator consumes the fee authorization by transferring directly to the escrow instead of calling `fund`, the reconciler finds that exact buyer-and-nonce event, waits for the same refund boundary, and requests a primary-quorum orphan refund before unpaid cancellation. Each attester reconstructs both authorizations and verifies that the token's matching `AuthorizationUsed` log is immediately followed by the exact buyer-to-escrow transfer. The contract requires the nonce to be consumed and pays only from balance above `totalEscrowedAtomic`, so the recovery cannot consume a normally recorded fee. If both immediate and scheduled capture miss the normal boundary, the buyer refunds and PolicyPool forfeits its fee even though the provider may have completed; this is a fee-collection liveness risk, not buyer fund loss. The escrow has no sweep or alternate recipient and verifies exact balance deltas.
...
Every receipt persists an immutable sidecar binding its issuance-time receipt hash and exact provider-relay endpoint, or an explicit null endpoint for non-relay records. A lifecycle transition cannot replace that sidecar, and Redis backfill never decodes and re-encodes the hash-committed receipt. The checked-in migration manifest pins every public receipt that predated sidecar enforcement. Any additional pre-anchor receipt fails closed unless the operator supplies a bounded `POLICYPOOL_RECEIPT_INTEGRITY_MIGRATIONS` entry captured from an independently trusted predeploy snapshot. Each entry binds one exact receipt ID, hash, and optional relay endpoint; generic current, legacy, or self-hosted route membership is not migration evidence. Relay backfill additionally rechecks durable cohort identity, timestamp, settlement transaction and amount, provider-bond cap, covenant, and relay-grant bindings before persisting the sidecar.
...
The remediated custody/state-transition suite passes 122 Foundry tests and includes executable hostile regressions for stale settlement, held breach evidence, terminal recovery, late completion, provisional-breach correction, primary and emergency challenge-period ordering, quorum separation, delayed relay-clock recovery with non-overlapping expiry, exact outbound token transfers, primary and recovery-quorum expired-unused fee cancellation, uncertain issuance reconciliation, the direct refundable fee escrow, directly consumed fee recovery, stale or under-threshold orphan evidence, and protection of normally accounted fees.
