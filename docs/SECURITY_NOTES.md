...
Relay receipts persist an immutable sidecar binding their issuance-time receipt hash and exact provider-relay endpoint. A lifecycle transition cannot replace that sidecar, and Redis backfill never decodes and re-encodes the hash-committed receipt. A pre-anchor receipt fails closed unless the operator supplies a bounded `POLICYPOOL_RECEIPT_INTEGRITY_MIGRATIONS` entry captured from an independently trusted predeploy snapshot. Each entry binds one exact receipt ID, hash, and relay endpoint; generic current, legacy, or self-hosted route membership is not migration evidence. Backfill also rechecks the durable timestamp, settlement transaction and amount, provider-bond cap, covenant, and relay-grant bindings before persisting the sidecar.
