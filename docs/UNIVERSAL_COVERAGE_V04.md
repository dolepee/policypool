...
- Recovers a fee authorization that an x402 facilitator settled directly to the escrow, outside `fund`, only after the primary quorum verifies the exact authorization nonce and ordered USD₮0 transfer; normally accounted escrow balances cannot fund that recovery.
